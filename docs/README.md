...
- **Dynamic & Persistent Configuration**: All configurations (API keys, auth tokens, etc.) are stored in a persistent database and can be modified on-the-fly via the Web UI without requiring an application restart.
...
This project is optimized for a seamless, one-click deployment experience on **Vercel**.

### Deployment Principles

- **Serverless & Fully Managed**: Vercel handles the entire build process, deployment, and hosting. You don't need to manage servers or Docker containers.
- **Integrated Database**: The project is configured to use **Vercel Postgres (Neon)**. Vercel automatically provisions the database and injects the necessary connection strings as environment variables during the build and runtime.
- **Automated Migrations**: Vercel's build process will automatically run `prisma migrate deploy` to ensure your database schema is always up-to-date with your `prisma/schema.prisma` file.

### Running on Vercel

1.  **Fork the Repository**: Click the "Fork" button at the top-right of this page to create your own copy of the project.

2.  **Deploy to Vercel**: Click the button below to deploy your forked repository to Vercel.

    [![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2FYOUR_USERNAME%2Fgemini-balance-nextjs-vercel&project-name=gemini-balance-nextjs&repository-name=gemini-balance-nextjs-vercel)

    *Remember to replace `YOUR_USERNAME` in the URL above with your actual GitHub username.*

3.  **Add Vercel Postgres Integration**: During the import process, Vercel will prompt you to add integrations. Select **Vercel Postgres** and follow the instructions to create a new database. Vercel will automatically link it to your project and set the following environment variables:
    - `POSTGRES_PRISMA_URL`
    - `POSTGRES_URL_NON_POOLING`

4.  **Configure Environment Variables**: In your Vercel project dashboard, navigate to **Settings > Environment Variables**. Add any other necessary variables, such as:
    - `CRON_SECRET`: A secure, random string to protect the health-check cron job endpoint.

5.  **Deploy**: Vercel will automatically build and deploy your application.

6.  **Perform First-Time Setup**: Once deployed, access your Vercel application URL (e.g., `https://your-project.vercel.app`) and follow the same "First-Time Setup via Web UI" steps as in the local development guide.
...
- For **Vercel deployment**, add it in the project's Environment Variables settings.

Choose a long, random, and hard-to-guess string for this value.

### 2. Configure Your Cron Service

You need to set up a cron job on your server or using a third-party service (like `cron-job.org` or Vercel Cron Jobs).
